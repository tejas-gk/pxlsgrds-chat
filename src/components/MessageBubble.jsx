...
export default function MessageBubble({ role, text, onRegenerate }) {
  ...
  const isHtml = useMemo(() => {
    return /<html|<body|<!DOCTYPE html|<head|<script|<style/i.test(text)
  }, [text])
  ...
  return (
    ...
    >
      <div
        className={`relative px-4 py-3 rounded-2xl max-w-2xl whitespace-pre-line shadow-sm ${
          isUser
          ...
          components={{
            code({ inline, className, children, ...props }) {
              ...
              return !inline && match ? (
                ...
                <code
                  className="bg-zinc-700 text-zinc-100 px-1 rounded text-sm"
                  {...props}
                >
                  {children}
                </code>
